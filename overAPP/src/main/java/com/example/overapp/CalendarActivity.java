...
import com.example.overapp.Utils.NormalSpan;
...
//初始化点击事件
        ...
        final List<MyDate> myDateList = LitePal.where("userId = ?", ConfigData.getNumLogged() + "").find(MyDate.class);
...
//像日历中添加框
    ...
    private void updateData(CalendarDay date) {
        List<MyDate> myDates = LitePal.where("date = ? and month = ? and year = ? and userId = ?", date.getDay() + "", (date.getMonth() + 1) + "", date.getYear() + "", ConfigData.getNumLogged() + "").find(MyDate.class);
        if (myDates.isEmpty()) {
