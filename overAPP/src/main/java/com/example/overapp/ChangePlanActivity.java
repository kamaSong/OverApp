...
import com.example.overapp.Utils.FileUtil;
import com.example.overapp.Utils.JsonHelper;
        ...
        public void handleMessage(@NonNull Message msg) {
            switch (msg.what) {
                case FINISH:
                    ...
                    userConfig1.updateAll("userId = ?", ConfigData.getNumLogged() + "");
                    // 删除当天打卡记录
...
//        初始化
                    ...
                    if (Integer.parseInt(editText.getText().toString().trim()) >= 5
                        ...
                        if (ConfigData.notUpdate == intent.getIntExtra(ConfigData.UPDATE_NAME, 0)) {
                                        ...
                                        public void run() {
                                            try {
                                                ...
                                                Request request = new Request.Builder()
                                                        .url(ConstantData.bookDownLoadAddress(currentBookId))
                                                        .build();
                                                Response response = client.newCall(request).execute();
                                                Message message = new Message();
                                                message.what = DOWN_DONE;
                                                handler.sendMessage(message);
                                                FileUtil.getFileByBytes(response.body().bytes(), getFilesDir()+ "/" + ConstantData.DIR_TOTAL, ConstantData.bookFileName(currentBookId));
                                                FileUtil.unZipFile(getFilesDir()+ "/" + ConstantData.DIR_TOTAL + "/" + ConstantData.bookFileName(currentBookId)
                                                        , getFilesDir() + "/" + ConstantData.DIR_TOTAL + "/" + ConstantData.DIR_AFTER_FINISH, false);
                                            } catch (Exception e) {

                                            }
                                            JsonHelper.analyseDefaultAndSave(FileUtil.readLocalJson(ConstantData.DIR_TOTAL + "/" + ConstantData.DIR_AFTER_FINISH + "/" + ConstantData.bookFileName(currentBookId).replace(".zip", ".json")));
                                            Message message = new Message();
                                            message.what = FINISH;
                                            handler.sendMessage(message);
                                        }
                            ...
                            if (userConfigs.get(0).getWordNeedReciteNum() != Integer.parseInt(editText.getText().toString().trim())) {
                                ...
                                userConfig1.updateAll("userId = ?", ConfigData.getNumLogged() + "");
                                Toast.makeText(ChangePlanActivity.this, "" + LitePal.where("userId = ?", ConfigData.getNumLogged() + "").find(UserConfig.class).get(0).getLastStartTime(), Toast.LENGTH_SHORT).show();
                                Log.d(TAG, "onClick: " + LitePal.where("userId = ?", ConfigData.getNumLogged() + "").find(UserConfig.class).get(0).getLastStartTime());
                                // 删除当天打卡记录
                                Calendar calendar = Calendar.getInstance();
                                LitePal.deleteAll(MyDate.class, "year = ? and month = ? and date = ? and userId = ?"
                                        , calendar.get(Calendar.YEAR) + ""
    ...
    protected void onStart() {
        ...
        maxNum = ConstantData.wordTotalNumber(currentBookId);
